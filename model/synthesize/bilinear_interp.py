...
class BilinearInterpolation:
    ...
    def __call__(self, image, pixel_coords, valid_mask=None):
        """
        :param image: source image [batch, numsrc, height, width, ?]
        :param pixel_coords: floating-point pixel coordinates (u,v,1) [batch, numsrc, 3 or 2, height*width]
        :param valid_mask: zero pixels in valid_mask are INVALID in recon_image [batch, height, width, 1]
        :return: reconstructed image [batch, numsrc, height, width, ?]
        """
        batch, numsrc, height, width, channels = image.get_shape()

        # pixel_floorceil[batch, numsrc, :, height*width] = (u_ceil, u_floor, v_ceil, v_floor)
        pixel_floorceil = self.neighbor_int_pixels(pixel_coords, height, width)

        # valid_mask: [batch, numsrc, 1, height*width]
        valid_mask2 = self.make_valid_mask(pixel_floorceil, valid_mask, batch)

        # weights[batch, numsrc, :, height*width] = (w_uf_vf, w_uf_vc, w_uc_vf, w_uc_vc)
        weights = self.calc_neighbor_weights([pixel_coords, pixel_floorceil, valid_mask2])

        # sampled_image[batch, numsrc, :, height, width, 3] =
        # (im_uf_vf, im_uf_vc, im_uc_vf, im_uc_vc)
        sampled_images = self.sample_neighbor_images([image, pixel_floorceil])

        # recon_image[batch, numsrc, height*width, 3]
        flat_image = self.merge_images([sampled_images, weights])
        recon_image = tf.reshape(flat_image, shape=(batch, numsrc, height, width, channels))
        return recon_image

    def neighbor_int_pixels(self, pixel_coords, height, width):
        """
        :param pixel_coords: (u, v) [batch, numsrc, 2, height*width]
        :param height: image height
        :param width: image width
        :return: (u_floor, u_ceil, v_floor, v_ceil) [batch, numsrc, 4, height*width]
        """
    ...
    def make_valid_mask(self, pixel_floorceil, valid_mask, batch):
        """
        :param pixel_floorceil: (u_floor, u_ceil, v_floor, v_ceil) (int) [batch, numsrc, 4, height*width]
        :param valid_mask: zero pixels in valid_mask are INVALID in recon_image [batch, height, width, 1]
        :param batch: batch size
        :return: mask [batch, numsrc, 1, height*width]
        """
        ...
        # mask [batch, numsrc, 1, height*width]
    ...
    def calc_neighbor_weights(self, inputs):
        ...
        """
        pixel_coords: (u, v) (float) [batch, numsrc, 2, height*width]
        pixel_floorceil: (u_floor, u_ceil, v_floor, v_ceil) (int) [batch, numsrc, 4, height*width]
        valid_mask: [batch, numsrc, 1, height*width]
        return: weights of four neighbor pixels (w_uf_vf, w_uf_vc, w_uc_vf, w_uc_vc) 
                [batch, numsrc, 4, height*width]
        """
        ui, vi = (0, 1)
        uf, uc, vf, vc = (0, 1, 2, 3)
    ...
    def sample_neighbor_images(self, inputs):
        ...
        """
        source_image: [batch, numsrc, height, width, 3]
        pixel_floorceil: (u_floor, u_ceil, v_floor, v_ceil) [batch, numsrc, 4, height*width]
        return: flattened sampled image [(batch, numsrc, 4, height*width, 3)]
        """
        ...
        """
        # tf.stack([vf, uf]): [batch, numsrc, height*width, 2(v,u)]
        ...
        # sampled_images: (batch, numsrc, 4, height*width, 3)
    ...
    def merge_images(self, inputs):
        ...
        """
        sampled_images: flattened sampled image [batch, numsrc, 4, height*width, 3]
        weights: 4 neighbor pixel weights (w_uf_vf, w_uf_vc, w_uc_vf, w_uc_vc) 
                 [batch, numsrc, 4, height*width]
        return: merged_flat_image, [batch, numsrc, height*width, 3]
        """
    ...
    def erase_invalid_pixels(self, inputs):
        ...
        """
        flat_image: [batch, numsrc, height*width, 3]
        depth: target view depth [batch, height, width, 1]
        return: [batch, numsrc, height*width, 3]
        """
...
class FlowBilinearInterpolation:
    ...
    def __call__(self, image, flow):
        """
        :param image: source image [batch*numsrc, height, width, ?]
        :param flow: optical flow [batch*numsrc, height, width, 2(u,v)]
        :return: reconstructed image [batch*numsrc, height, width, ?]
        """
        # -> [batch*numsrc, 1, height, width, ?]
    ...
    def flow_to_pixel_coordinates(self, flow):
        """
        :param flow: optical flow [batch, numsrc, height, width, 2(u,v)]
        :return: pixel_coords: source image pixel coordinates [batch, numsrc, 2, height*width]
        """
