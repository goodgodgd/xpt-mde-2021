...
class PhotometricLossMultiScale(LossBase):
    ...
    def photometric_loss(self, synt_target, orig_target):
        raise NotImplementedError()


class PhotometricLossL1MultiScale(PhotometricLossMultiScale):
    @shape_check
    def photometric_loss(self, synt_target, orig_target):
        """
...
class PhotometricLossSSIMMultiScale(PhotometricLossMultiScale):
    @shape_check
    def photometric_loss(self, synt_target, orig_target):
        """
        :param synt_target: scaled synthesized target image [batch, num_src, height/scale, width/scale, 3]
